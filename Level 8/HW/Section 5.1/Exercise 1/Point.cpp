...
namespace JoseAponte
{
	namespace CAD
	{
		...
		{
			cout << "Point destructor called." << endl;							//Printing the following to make sure shared_pointers are deleted automatically
		}

		Point::Point(double x, double y) : Shape(), m_x(x), m_y(y) {}			//Constructor that accepts x- and y-coordinates with Colon Syntax

		Point::Point(double single) : Shape(), m_x(single), m_y(single) {}		//Constructor with a single parameter that sets m_x and m_y to same coordiate
		...
		void Point::Draw() const											//Point Draw () function that simulates drawing by just printing some text
		{
