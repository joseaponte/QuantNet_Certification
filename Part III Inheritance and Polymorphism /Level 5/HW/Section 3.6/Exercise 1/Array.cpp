...
namespace JoseAponte
{
	namespace Containers
	{
		...
		Point& Array::operator[](int index)				//Square bracket operator
		{
			...
			{
				throw - 1;									//Throw -1 if the index is out of bounds
			}
		}
		...
		int Array::Size() const								//Size() function that returns the size of the array
		{
			return arr_size;
		}
		...
		Point& Array::GetElement(int i) const				//GetElement() function that gets an element
		{
			if (i >= 0 && i < arr_size)						//When the data index is in bounds
			{
