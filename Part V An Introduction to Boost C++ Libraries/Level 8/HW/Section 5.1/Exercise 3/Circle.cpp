...
namespace JoseAponte
{
	namespace CAD
	{
		...
		Circle::Circle(const Point& P, const Line& L) : Shape(), center(P), radius(L) {}		//Constructor that accepts a line and a point as parameters w/ Colon Syntax
		...
		void Circle::Center_Point(const Point& P)	//Center point of Circle
		{
			center = P;
		}

		void Circle::Radius(const Line& L)			//Radius of Circle
		{
			radius = L;
		}
		...
		{
			os << "Circle:(Center:" << C.Center_Point() << "; Radius:" << C.Radius() << ")";		//Prints description of circle
			return os;
		}
